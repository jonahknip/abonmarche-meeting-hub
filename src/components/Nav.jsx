...
import { Search, Upload, MessageCircle, ChevronLeft, Menu, X, Calendar, Mic } from 'lucide-react'

export default function Nav({ view, setView, onUpload, onRecord, onChat, searchQuery, setSearchQuery }) {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled || view !== 'home' ? 'glass' : 'bg-transparent'}`}>
      <div className="max-w-6xl mx-auto px-5 h-[52px] flex items-center gap-4">
        ...
        {view !== 'home' && (
          <button onClick={() => setView(view === 'detail' ? 'dashboard' : 'home')}
        ...
        <button onClick={() => setView('home')} className="flex items-center gap-2.5 mr-auto">
          ...
          <span className="text-[15px] font-semibold tracking-tight text-[#003087]">MHAI</span>
          <span className="hidden sm:block text-[11px] text-gray-400 font-normal mt-px">Abonmarche Intelligence</span>
        </button>
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          <NavBtn active={view === 'calendar'} onClick={() => setView('calendar')}>
            <Calendar size={12} className="mr-1" />Calendar
          </NavBtn>
          <NavBtn active={view === 'week'} onClick={() => setView('week')}>Week One</NavBtn>
          <button onClick={onChat}
            ...
            <MessageCircle size={13} />Ask MHAI
          </button>
          <button onClick={onRecord}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-500 text-white text-[13px] font-medium hover:bg-red-600 transition-colors ml-1 animate-pulse-subtle">
            <Mic size={13} />Record
          </button>
      ...
      {mobileOpen && (
        <div className="md:hidden glass border-t border-black/5 px-5 py-3 space-y-2">
          ...
          <MobileBtn onClick={() => { setView('calendar'); setMobileOpen(false) }}>Calendar</MobileBtn>
          ...
          <MobileBtn onClick={() => { onRecord(); setMobileOpen(false) }}>Record Meeting</MobileBtn>
...
function NavBtn({ active, onClick, children }) {
  return (
    <button onClick={onClick}
      className={`flex items-center px-3 py-1.5 rounded-full text-[13px] font-medium transition-colors ${active ? 'bg-black/8 text-gray-900' : 'text-gray-600 hover:text-gray-900 hover:bg-black/5'}`}>
      {children}
    </button>
  )
}
